...
static int put_image(struct vf_instance *vf, mp_image_t *mpi, double pts){
	...
	if ((vf->priv->startpos < -0.1f && vf->priv->endpos < -0.1f) ||
		(vf->priv->startpos > -0.1f && pts < vf->priv->startpos) ||
		(vf->priv->endpos > -0.1f && pts > vf->priv->endpos) ) {
		// No need to delogo
		dmpi = mpi;
	} else {
	    fprintf(stderr, "Pts:%1.3s===========\n", pts);
...
#endif
    { "start", ST_OFF(startpos), CONF_TYPE_FLOAT, 0, 0, 0, NULL },
    { "end", ST_OFF(endpos), CONF_TYPE_FLOAT, 0, 0, 0, NULL },
...
const vf_info_t vf_info_delogo = {
    "simple logo remover",
    "delogo",
    "Jindrich Makovicka, Alex Beregszaszi, Stanley Huang",
    "",
    vf_open,
    &vf_opts
};
